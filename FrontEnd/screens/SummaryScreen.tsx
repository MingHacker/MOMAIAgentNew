import React, { useEffect, useState } from 'react';
import { 
  View, 
  Text,
  StyleSheet, 
  ScrollView, 
  Dimensions,
  TouchableOpacity,
  SafeAreaView 
} from 'react-native';
import { LineChart } from 'react-native-chart-kit';
import { BarChart } from 'react-native-chart-kit';
import BabyStatusCard from '../components/BabyStatusCard'; 
import MomStatusCard from '../components/MomStatusCard';
import RecordModal from '../components/RecordModal';
import { momApi } from '../src/api';  // Make sure this is imported
import { babyApi } from '../src/api';  // Make sure this is imported
import AsyncStorage from '@react-native-async-storage/async-storage';
import MoodSelector from '../components/MoodSelector';

import dayjs from 'dayjs';

// 设备宽度，用于控制图表大小
const screenWidth = Dimensions.get('window').width;


// x轴标签（示例：一周）
const weekLabels = ['Mon','Tue','Wed','Thu','Fri','Sat','Sun'];

export default function HealthSummaryScreen() {
  const [modalVisible, setModalVisible] = useState(false);
  const [selectedType, setSelectedType] = useState<'feeding' | 'sleep' | 'diaper' | 'outside' | null>(null);
  const [formData, setFormData] = useState<any>({});
  const [submitting, setSubmitting] = useState(false);
  const [babyData, setBabyData] = useState<any>(null);
  const [momData, setMomData] = useState<any>(null);
  const [babySummary, setBabySummary] = useState<string | null>(null);
  const [momSummary, setMomSummary] = useState<string | null>(null);
  const [mood, setMood] = useState<string | null>(null); 

  // 获取妈妈的健康数据
  useEffect(() => {
    const fetchMomData = async () => {
      try {
        const response = await momApi.getWeeklyHealth(); // 使用真实API
        if (response.success && response.data) {
          setMomData(response.data);  // 格式化后的数据
        }
      } catch (error) {
        console.error('获取妈妈的健康数据失败:', error);
      }
    };

    fetchMomData();
  }, []); // 空依赖数组，组件首次加载时获取数据

  // 获取宝宝的健康数据（你可以在这里保留现有的数据或使用真实API）
  useEffect(() => {
    const fetchBabyData = async () => {
      try {
        const babyId = await AsyncStorage.getItem('baby_id') || '';
        const response = await babyApi.getWeeklySummary(babyId); // 获取宝宝每周总结数据
        if (response.success && response.data) {
          setBabyData(response.data);  // 格式化后的数据
        }
      } catch (error) {
        console.error('获取宝宝的健康数据失败:', error);
      }
    };

    fetchBabyData();
  }, []);

  // 添加提交记录的函数
  const submitRecord = async (data: any) => {
    try {
      setSubmitting(true);
      const babyId = await AsyncStorage.getItem('baby_id');
      if (!babyId) {
        console.error('No baby ID found');
        return;
      }

      // 根据 selectedType 提交不同的记录
      switch (selectedType) {
        case 'feeding':
          await babyApi.createBabyLog({
            baby_id: babyId,
            log_type: 'feeding',
            log_data: data
          });
          break;
        case 'sleep':
          await babyApi.createBabyLog({
            baby_id: babyId,
            log_type: 'sleep',
            log_data: data
          });
          break;
        case 'diaper':
          await babyApi.createBabyLog({
            baby_id: babyId,
            log_type: 'diaper',
            log_data: data
          });
          break;
        case 'outside':
          await babyApi.createBabyLog({
            baby_id: babyId,
            log_type: 'outside',
            log_data: data
          });
          break;
      }

      // 关闭模态框并重置状态
      setModalVisible(false);
      setSelectedType(null);
      setFormData({});
      
      // 刷新数据
      // 这里可以添加刷新数据的逻辑
      
    } catch (error) {
      console.error('Error submitting record:', error);
    } finally {
      setSubmitting(false);
    }
  };

  if (!momData) {
    return (
      <SafeAreaView style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
        <Text>Loading...</Text>
      </SafeAreaView>
    );
  }
  // 图表配置
  const chartConfig = {
    backgroundGradientFrom: '#fff',
    backgroundGradientTo: '#fff',
    color: (opacity = 1) => `rgba(120, 70, 250, ${opacity})`, 
    labelColor: (opacity = 1) => `rgba(60, 60, 60, ${opacity})`,
    strokeWidth: 2,
    barPercentage: 0.5,
  };

  return (
    <SafeAreaView style={[styles.safeArea, { paddingBottom: 30 }]}>
      <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 20 }}>
        <BabyStatusCard onStartRecording={() => {
          console.log('Opening modal...'); // 添加调试日志
          setModalVisible(true);
        }} />
        <MomStatusCard summary={momSummary} />
        <MoodSelector onMoodSelect={(selectedMood: string) => setMood(selectedMood)} />


        {/* Baby Feeding Card */}
        <View style={styles.card}>
          <View style={styles.cardHeader}>
            <Text style={styles.cardHeaderTitle}>Baby Feeding (mL)</Text>
            <Text style={styles.cardHeaderSubtitle}>
              Today {babyData?.[6]?.feed_total_ml ?? '--'} ml</Text>
          </View>
          <LineChart
            data={{
              labels: weekLabels,
              datasets: [
                { data: babyData?.map((item) => item.feed_total_ml) ?? [0] }, // 用真实数据
              ],
            }}
            width={screenWidth * 0.9}
            height={150}
            chartConfig={chartConfig}
            style={styles.chartStyle}
            bezier
          />
        </View>

        {/* Baby Sleep Card */}
        <View style={styles.card}>
              <View style={styles.cardHeader}>
                <Text style={styles.cardHeaderTitle}>Sleep</Text>
                <Text style={styles.cardHeaderSubtitle}>
                  Today {babyData?.[6]?.sleep_total_hours ?? '--'} hr</Text>
              </View>
              <LineChart
                data={{
                  labels: weekLabels,
                  datasets: [
                    { data: babyData?.map((item) => item.sleep_total_hours) ?? [0] },
                  ],
                }}
                width={screenWidth * 0.9}
                height={150}
                chartConfig={chartConfig}
                style={styles.chartStyle}
                bezier
              />
            </View>

            {/* Baby Diaper Card */}
            <View style={styles.card}>
              <View style={styles.cardHeader}>
                <Text style={styles.cardHeaderTitle}>Diaper</Text>
                <Text style={styles.cardHeaderSubtitle}>
                  Today {babyData?.[6]?.diaper_count ?? '--'} times</Text>
              </View>
              <BarChart
                data={{
                  labels: weekLabels,
                  datasets: [
                    { data: babyData?.map((item) => item.diaper_count) ?? [0] },
                  ],
                }}
                width={screenWidth * 0.9}
                height={150}
                chartConfig={chartConfig}
                style={styles.chartStyle}
          
              />
            </View>

            {/* Baby Diaper Card */}
            <View style={styles.card}>
              <View style={styles.cardHeader}>
                <Text style={styles.cardHeaderTitle}>Diaper Solid</Text>
                <Text style={styles.cardHeaderSubtitle}>
                  Today {babyData?.[6]?.bowel_count ?? '--'} times</Text>
              </View>
              <BarChart
                data={{
                  labels: weekLabels,
                  datasets: [
                    { data: babyData?.map((item) => item.diaper_count) ?? [0] },
                  ],
                }}
                width={screenWidth * 0.9}
                height={150}
                chartConfig={chartConfig}
                style={styles.chartStyle}
              />
            </View>

            {/* Baby Outside Card */}
            <View style={styles.card}>
              <View style={styles.cardHeader}>
                <Text style={styles.cardHeaderTitle}>Outside</Text>
                <Text style={styles.cardHeaderSubtitle}>
                  Today {babyData?.[6]?.outside_total_minutes ?? '--'} min</Text>
              </View>

              <BarChart
                data={{
                  labels: weekLabels,
                  datasets: [
                    { data: babyData?.map((item) => item.outside_total_minutes) ?? [0] },
                  ],
                }}
                width={screenWidth * 0.9}
                height={150}
                chartConfig={chartConfig}
                style={styles.chartStyle}
              />
            </View>

            {/* Baby Cry Card */}
            {babyData?.some(item => item?.cry_total_minutes && item.cry_total_minutes > 0) && (
              <View style={styles.card}>
                <View style={styles.cardHeader}>
                  <Text style={styles.cardHeaderTitle}>Cry</Text>
                  <Text style={styles.cardHeaderSubtitle}>
                    Today {babyData?.[6]?.cry_total_minutes ?? '--'} min
                  </Text>
                </View>
                <BarChart
                  data={{
                    labels: weekLabels,
                    datasets: [
                      { data: babyData?.map((item) => item.cry_total_minutes) ?? [0] },
                    ],
                  }}
                  width={screenWidth * 0.9}
                  height={150}
                  chartConfig={chartConfig}
                  style={styles.chartStyle}
                />
              </View>
            )}

        <Text style={styles.headerTitle}>MOM Summary</Text>


        {/* Mom HRV Card - 只在有 HRV 数据时显示 */}
        {momData?.some(item => item?.hrv) && (
          <View style={styles.card}>
            <View style={styles.cardHeader}>
              <Text style={styles.cardHeaderTitle}>Heart Rate (HR)</Text>
              <Text style={styles.cardHeaderSubtitle}>
                Today {momData?.[6]?.resting_heart_rate ?? '--'} bpm
              </Text>
            </View>
            <LineChart
              data={{
                labels: weekLabels,
                datasets: [
                  { data: momData?.map((item) => item?.resting_heart_rate ?? 0) ?? [0] },
                ],
              }}
              width={screenWidth * 0.9}
              height={150}
              chartConfig={chartConfig}
              style={styles.chartStyle}
              bezier
            />
          </View>
        )}

        {/* Mom Sleep Card - 只在有睡眠数据时显示 */}
        {momData?.some(item => item?.sleep_hours) && (
          <View style={styles.card}>
            <View style={styles.cardHeader}>
              <Text style={styles.cardHeaderTitle}>Sleep</Text>
              <Text style={styles.cardHeaderSubtitle}>
                Today {momData?.[6]?.sleep_hours ?? '--'} hours
              </Text>
            </View>
            <LineChart
              data={{
                labels: weekLabels,
                datasets: [
                  { data: momData?.map((item) => item?.sleep_hours ?? 0) ?? [0] },
                ],
              }}
              width={screenWidth * 0.9}
              height={150}
              chartConfig={chartConfig}
              style={styles.chartStyle}
              bezier
            />
          </View>
        )}

        {/* HRV Card - 只在有 HRV 数据时显示 */}
        {momData?.some(item => item?.hrv) && (
          <View style={styles.card}>
            <View style={styles.cardHeader}>
              <Text style={styles.cardHeaderTitle}>HRV</Text>
              <Text style={styles.cardHeaderSubtitle}>
                Today {momData?.[6]?.hrv ?? '--'}
              </Text>
            </View>
            <LineChart
              data={{
                labels: weekLabels,
                datasets: [
                  { data: momData?.map((item) => item?.hrv ?? 0) ?? [0] },
                ],
              }}
              width={screenWidth * 0.9}
              height={150}
              chartConfig={chartConfig}
              style={styles.chartStyle}
              bezier
            />
          </View>
        )}

        {/* Step Card - 只在有步数数据时显示 */}
        {momData?.some(item => item?.steps) && (
          <View style={styles.card}>
            <View style={styles.cardHeader}>
              <Text style={styles.cardHeaderTitle}>Step</Text>
              <Text style={styles.cardHeaderSubtitle}>
                Today {momData?.[6]?.steps ?? '--'}
              </Text>
            </View>
            <LineChart
              data={{
                labels: weekLabels,
                datasets: [
                  { data: momData?.map((item) => item?.steps ?? 0) ?? [0] },
                ],
              }}
              width={screenWidth * 0.9}
              height={150}
              chartConfig={chartConfig}
              style={styles.chartStyle}
              bezier
            />
          </View>
        )}

        {/* Resting HR Card - 只在有心率数据时显示 */}
        {momData?.some(item => item?.resting_heart_rate) && (
          <View style={styles.card}>
            <View style={styles.cardHeader}>
              <Text style={styles.cardHeaderTitle}>Resting HR</Text>
              <Text style={styles.cardHeaderSubtitle}>
                Today {momData?.[6]?.resting_heart_rate ?? '--'}
              </Text>
            </View>
            <LineChart
              data={{
                labels: weekLabels,
                datasets: [
                  { data: momData?.map((item) => item?.resting_heart_rate ?? 0) ?? [0] },
                ],
              }}
              width={screenWidth * 0.9}
              height={150}
              chartConfig={chartConfig}
              style={styles.chartStyle}
              bezier
            />
          </View>
        )}
        
        {/* Calories Burned Card - 只在有卡路里数据时显示 */}
        {momData?.some(item => item?.calories_burned) && (
          <View style={styles.card}>
            <View style={styles.cardHeader}>
              <Text style={styles.cardHeaderTitle}>Calories Burned</Text>
              <Text style={styles.cardHeaderSubtitle}>
                Today {momData?.[6]?.calories_burned ?? '--'}
              </Text>
            </View>
            <LineChart
              data={{
                labels: weekLabels,
                datasets: [
                  { data: momData?.map((item) => item?.calories_burned ?? 0) ?? [0] },
                ],
              }}
              width={screenWidth * 0.9}
              height={150}
              chartConfig={chartConfig}
              style={styles.chartStyle}
              bezier
            />
          </View>
        )}

        <RecordModal
          visible={modalVisible}
          onClose={() => {
            setModalVisible(false);
            setSelectedType(null);
            setFormData({});
          }}
          selectedType={selectedType}
          setSelectedType={setSelectedType}
          formData={formData}
          setFormData={setFormData}
          onSubmit={submitRecord}
          submitting={submitting}
        />
      </ScrollView>
    </SafeAreaView>
  );
}

const styles = StyleSheet.create({
  safeArea: {
    flex: 1,
    backgroundColor: '#FFF',
  },
  container: {
    flex: 1,
    paddingTop: 10,
  },

  headerTitle: {
    fontSize: 16,
    fontWeight: '600',
    marginLeft: 16,
    marginTop: 16,
    marginBottom: 4,
  },

  card: {
    backgroundColor: '#f8f8f8',
    borderRadius: 12,
    paddingVertical: 12,
    marginHorizontal: 12,
    marginBottom: 12,
    alignItems: 'center',
  },

  cardHeader: {
    width: '90%',
    marginBottom: 8,
  },
  cardHeaderTitle: {
    fontSize: 16,
    fontWeight: 'bold',
  },
  cardHeaderSubtitle: {
    fontSize: 12,
    color: '#666',
    marginTop: 2,
  },
  chartStyle: {
    borderRadius: 8,
    marginLeft: -10,
    paddingBottom: 5,
  },
});

